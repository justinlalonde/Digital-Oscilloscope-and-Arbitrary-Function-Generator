...
## Testing
Each submodule was verified individually with self-checking VHDL testbenches, simulated in GHDL and cross-checked in Vivado, before being integrated into the top-level design. End-to-end validation was then carried out on the physical Basys 3 setup with the Pmod AD1 and DA2 boards, checking signal acquisition and ETS reconstruction against known reference signals, cursor voltage readouts, and function generator output accuracy across the supported signal types, frequencies, and amplitudes.

## Future Improvements
- Hybrid ETS/real-time-sampling (RTS) scheme with decimation for signal reconstruction, chosen adaptively based on signal frequency
- A second display for showing measurement values such as cursor voltages, signal frequency, and the oscilloscope's current control mode
- An expanded analog front end supporting a full ±10V measurement range, with signal conditioning down to the board's native 0V–3.3V range
- instead of a power of 2 based frequency, amlitude , vertical zoom and horizontal zoom adjustement functionality, implement a more gradual and precise way of modifying these user parameters with rotary encoders or keyboard extensions.
